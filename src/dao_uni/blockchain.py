...
class Blockchain:
    ...
    def transaction_exists(self, transaction):
        for token in self.chain:
            block = Block.verify_jwt(token, self.genesis_wallet.get_public_key_obj())
            if block and transaction in block["transactions"]:
                return True
        return False

    def list_transactions_by_type_or_origin(self, type_or_origin):
        transactions = []
        for token in self.chain:
            block = Block.verify_jwt(token, self.genesis_wallet.get_public_key_obj())
            if block:
                for transaction in block["transactions"]:
                    if type_or_origin in transaction:
...
if __name__ == "__main__":
    ...
    print("Transaction A-> B: 5 exists:", blockchain.transaction_exists("A-> B: 5"))
    print("Transactions involving A:", blockchain.list_transactions_by_type_or_origin("A"))
